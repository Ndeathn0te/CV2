...
import { portfolioDataEs, portfolioDataEn } from "./portfolioData";
...
export default function App() {
  ...
  const [view, setView] = useState<'cv' | 'portfolio'>('cv');
  ...
  const toggleView = () => setView(view === 'cv' ? 'portfolio' : 'cv');

  const cvData = lang === 'es' ? cvDataEs : cvDataEn;
  const portfolioData = lang === 'es' ? portfolioDataEs : portfolioDataEn;
  ...
  return (
    <div className={`${isDarkMode ? "dark" : ""} min-h-screen transition-colors duration-500 bg-neutral-100 dark:bg-dark-bg font-sans text-neutral-900 dark:text-neutral-100 p-2 md:p-8 print:p-0 print:bg-white overflow-x-hidden`}>
      ...
      <div className="fixed bottom-8 right-8 flex flex-col gap-4 print:hidden z-50">
        <motion.button 
          ...
          whileTap={{ scale: 0.9 }}
          onClick={toggleView}
          className="bg-black dark:bg-white text-white dark:text-black p-4 rounded-full shadow-2xl flex items-center justify-center border border-neutral-800 dark:border-neutral-200"
          title={view === 'cv' ? (lang === 'es' ? "Ver Portafolio" : "View Portfolio") : (lang === 'es' ? "Ver Currículum" : "View CV")}
        >
          {view === 'cv' ? <Briefcase size={20} /> : <User size={20} />}
        </motion.button>
        <motion.button 
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.9 }}
      ...
      <AnimatePresence mode="wait">
        {view === 'cv' ? (
          <motion.div 
            ...
            id="cv-print-area"
            ref={cvRef} 
            className="w-full md:w-[210mm] mx-auto bg-white dark:bg-dark-surface shadow-2xl print:shadow-none relative border border-neutral-200 dark:border-dark-border print:border-none min-h-[560mm] flex flex-col transition-colors duration-500 overflow-hidden"
          >
          ...
          <motion.div
            ...
            exit={{ opacity: 0, x: -20 }}
            className="w-full xl:w-[280mm] mx-auto min-h-screen"
          >
            <Portfolio projects={portfolioData} lang={lang} />
          </motion.div>
        )}
      </AnimatePresence>
